...
var ABSOLUTE            = 'absolute',
    ...
    URL_ABSOLUTE_REGEX  = /^(?:(https?):\/\/|\/\/)(?:([^:@\s]+:?[^:@\s]+?)@)?((?:[^;:@=\/\?\.\s]+\.)+[A-Za-z0-9\-]{1,})(?::(\d+))?(?=\/|\?|#|$)([^\?#]+)?(?:\?([^#]+))?(?:#(.+))?/i,
...
URL.prototype = {
    ...
    /**
     * Returns or sets the param(objectify) of the URL.
     * This takes or returns the parsed query as an Array of Arrays.
     *
     * @public
     * @method  param
     * @param   {String}  key   -
     * @param   {String}  val   - Optional query to set on the URL
     * @return  {String | URL}   the URL query or the URL instance
     */
    param: function (key, val) {
        var li = this._url[QUERY];
